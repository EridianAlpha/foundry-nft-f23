...
contract MintBasicNft is Script, DevOpsTools {
    function run() public {
        string memory jsonString =
            '{"name": "PUG", "description": "An adorable PUG pup!", "image": "ipfs://QmSsYRx3LpDAb1GZQm7zZ1AuHZjfbPkD6J7s9r41xu1mf8", "attributes": [{"trait_type": "cuteness", "value": 100}]}';

        string memory jsonBase64Encoded = Base64.encode(bytes(jsonString));

        string memory PUG_URI = string(abi.encodePacked("data:application/json;base64,", jsonBase64Encoded));

        address mostRecentDeployed = get_most_recent_deployment("BasicNft", block.chainid);
...
contract MintMoodNft is Script, DevOpsTools {
    function run() public {
        address mostRecentDeployed = get_most_recent_deployment("MoodNft", block.chainid);
